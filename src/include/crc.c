...
uint64_t crc_calc_table(char *data, size_t length, struct crc_info *info,
	...
	uint64_t reg = info->init_val;
	uint64_t reg_mask = __crc_gen_mask(info->pol_width * 8);

	if (info->init_val)
		for (uint8_t i = 0; i < info->pol_width * 8; i++)
			reg = reg & 1 ?
					 ((uint64_t)(reg ^ info->polynomial) >> (uint64_t)1 | (uint64_t)reg_mask ) : (reg >> (uint64_t)1);
...
// TODO: That black magic with inversed CRC32 when init_val != 0
	...
	if (info->init_val){
		...
		for (uint8_t i = 0; i < info->pol_width * 8; i++)
			reg = reg & 1 ?
					 ((uint64_t)(reg ^ info->polynomial) >> (uint64_t)1 | reg_mask ) : (reg >> (uint64_t)1);
	}else{
		reg = 0;
	}
	...
	for (size_t i = 0; i < length; i++) {
		for (uint8_t j = 0; j < 8; j++) {
			...
			if (info->ref_form) {
				uint64_t app_val = (uint64_t)curr_bit << (uint64_t)((info->pol_width << 3) - 1);
				reg = (reg & 1) ?
